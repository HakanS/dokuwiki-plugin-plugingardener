...
/**
 * Harvest the web (or depending on settings read the local files)
 */
class pg_dokuwikiwebexaminer extends pg_gardener {
     ...
     *
     * @return bool whether it retrieved plugins
     */
     ...
     *
     * @return array with pluginnames harvested from index
     */
    ...
    /**
     * Returns subset of array $plugins, starting at cfg['firstplugin'] until cfg['lastplugin']
     *
     * @param array $plugins complete array with plugin names
     * @return array with plugin names
     */
    private function _applyPluginLimits($plugins) {
        $retval = array();
     ...
     */
    private function _examineHomepages($plugins) {
    ...
    /**
     * Examine a plugin homepage
     *
     * @param string $plugin plugin name
     * @param string $markup content of plugin wiki page
     */
    private function _examinePluginHomepage($plugin, $markup) {
        ...
        //$page = explode('bar__bottom', $page[1]); //button bar at bottom in template 'default'
        $page = explode('<!-- /wrapper -->', $page[1]);
        $page = $page[0];
...
//<dd><a href="/plugin:sidebar" class="wikilink1" title="plugin:sidebar">sidebar</a></dd>
        if (preg_match('/\<dt>Conflicts with<\/dt>\n <dd\><(.*?)\<\/dd\>/', $page, $match)) {
    ...
    /**
     * Calculate some readibility scores
     *
     * @param string $plugin plugin name
     * @param string $page content of plugin wiki page
     */
    private function _readabilityIndex($plugin, $page) {
     ...
     */
    private function _getPopularityData() {
    ...
    /**
     * Retrieve the old plugins main index page to get the old popularity
     */
    private function _getOldPopularityData() {
    ...
    /**
     * Download repository of repo plugin to get best compatibility
     */
    private function _getPluginRepo() {
        echo "<li>Plugin repo ";
     ...
     *
     * @param  SimpleXMLElement|array $obj SimpleXMLElement or array of its properties
     * @return array|null
     */
    private function obj_array($obj) {
        $data = array();
        if (is_object($obj))
    ...
    /**
     * Download external plugin homepages and examine for urls
     */
    private function _examineExternalHomepages() {
        ...
        */
        ...
        foreach ($checkpages as $plugin => $pageurl) {
            ...
            if (($this->cfg['downloadpages'] || !file_exists($localpagedir.$plugin.'.external.htm')) && !$this->cfg['offline']) {
                echo "<li><a href=\"$this->cfg[doku_pluginbase_uri]$plugin\">$plugin</a> - <a href=\"$pageurl\">$pageurl</a></li>\n";
     ...
     *
     * @param string $plugin plugin name
     * @param string $page content of wiki page
     * @param string $domain
     */
    private function _examineLinks($plugin, $page, $domain) {
     ...
     *  stores download url and whether mediaurl
     *
     * @param string $plugin
     * @param string $url
     * @return bool whether download url
     */
    private function _getDownloadLink($plugin, $url) {
...
//        if (($this->cfg['downloadpages'] || !file_exists($localcopy)) && !$this->cfg['offline']) {
        ...
        $this->collections['eventlist'] = array();
